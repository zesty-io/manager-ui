...
SCREENSHOTS_DIR="/home/runner/work/manager-ui/manager-ui/cypress/screenshots/"
SCREENSHOTS=$(find $SCREENSHOTS_DIR -type f -name "*.png")

find $SCREENSHOTS_DIR -type f -name "*.png" | while read screenshot; do
    file=$(basename "$screenshot")
    echo "uploading $file"
    gsutil cp "$screenshot" gs://cypress_screenshots/"$file"
    if [ $? -ne 0 ]; then
