...
# apart once, here, so neither check below has to print the other's lines. A listed path occupies
# the whole line and never contains a space; every diagnostic line does. The two patterns are exact
# complements, so no line is silently dropped.
TSC_FILES=$(printf '%s\n' "$TSC_OUTPUT" | grep -E '^/[^ ]*$')
...
if [ -n "$MISSING_FILES" ]; then
    ...
    echo "the tsconfig.json program no longer covers our sources, so these files were never checked"
