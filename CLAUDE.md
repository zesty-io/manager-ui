...
Node is pinned to **22.19.0** via Volta and CI. `npm run lint` runs ESLint (`eslint src`, config in `eslint.config.js`) plus `scripts/lint-i18n-locales.js` for locale-JSON checks; it's required in CI (see Branch & PR flow). `pretty-quick` still runs on staged files via the husky `pre-commit` hook, separately from lint.
...
- **Cypress plus a lint gate.** `.github/workflows/ci.yaml` runs `npm run ci` (`start-server-and-test start … test`) and a separate `lint` job (`npm run lint` — ESLint, including `eslint-plugin-i18next` and this repo's own i18n key-format/locale-JSON checks); `all_tests_passed` requires both. There is still **no typecheck step and no build gate**. If you've touched TypeScript, run `npx tsc --noEmit` locally before opening a PR; the pipeline will not catch type errors for you.
