...
**Verification** (`npx tsc --noEmit` is covered repo-wide by ci.yaml's `typecheck` job, but run it yourself for faster feedback — CI still runs **no lint** and the Localization Reviewer workflow reports only localization findings, never generic TS errors): `npx tsc --noEmit`; JSON valid; key parity across all 6 locales (non-plural keys identical; plural keys carry each locale's full CLDR set).
...
- **Cypress plus a dedicated typecheck job are the test gates.** `.github/workflows/ci.yaml` runs `npm run ci` (`start-server-and-test start … test`) and a separate `typecheck` job (`npx tsc --noEmit`, repo-wide); both feed into the `all_tests_passed` aggregate check. There is still **no lint step, no build gate** — `pretty-quick` only runs pre-commit, and nothing runs `npm run build:*` in CI.
