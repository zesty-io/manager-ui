...
- **`claude-negative-qa.yml` drives the real app in a browser.** It boots the dev server, seeds a throwaway model/item via `ci/scripts/qa_seed.mjs`, hands Playwright MCP a pre-authenticated session through a `--storage-state` file (so the token never reaches the agent's prompt), and lets a senior-QA persona attack the surfaces the PR diff touches. Findings post as an upserted PR comment keyed on `<!-- negative-qa -->`, with screenshots pushed to `gs://cypress_screenshots/negative-qa/…`. It is **advisory and always exits 0**. It joins ci.yaml's `manager-ui-e2e-dev-instance` concurrency group because both mutate the same shared dev instance — if you add another job that writes to that instance, put it in the same group.
