...
- **Cypress is the only test gate.** `.github/workflows/ci.yaml` runs `npm run ci`, which is `start-server-and-test start … test` — it carries **no lint step, no typecheck step, no build gate**. If you've touched TypeScript, run `npm run typecheck` locally before opening a PR — it runs `tsc --noEmit` and fails on any diagnostic outside `node_modules/`; `ci.yaml` will not catch type errors for you.
