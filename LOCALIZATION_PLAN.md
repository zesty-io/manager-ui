...
**Done (sub-pass 3 implemented).** Localized ItemEdit chrome and the publish/save
critical path: header tab labels/tooltips/menus/dialogs, version/language/publish
status controls, `PublishState`, `LockedItem`, `PendingEditsModal`,
`Content/Content` sidebar/DUO tooltips, and `Content/Actions` widgets
(`ContentInfo`, `ContentLinks`, unpublish/delete/history/listed/CDN/share/workflow
request). Audited and localized in-scope notifications from legacy Redux/thunks
and RTK-triggered flows: `ItemEdit` save/load failures, duplicate failure, publish
status guard, CDN purge success/failure, workflow email success/failure, and
`navContent` fetch failure via the i18n singleton. Added 119 `itemEdit*` keys to
all 6 `content.json` files; key coverage verified across all locales. Left
future-scope strings in Meta/Redirects/Analytics/Head/APIs/Freestyle/Studio/block
secondary tabs for their dedicated sub-passes.

**Done (sub-pass 4 implemented).** Localized the ItemEdit SEO/meta tab:
meta flow picker, SEO/Open Graph/URL/advanced section copy, `Meta/settings`
field labels/tooltips/placeholders, social preview placeholders, content insight
metrics/empty states/toggles, and incoming redirects table/actions. Deep-checked
