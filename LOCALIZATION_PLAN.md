...
>
> **`FieldTypeRepeater/SubField.tsx` had leftover misses** (confirmed during the
> FieldTypeEditor pass): two hardcoded placeholders on its non-editor sub-fields
> — `"UUID field values are auto-generated"` and `"Select"`. Backfilled via
> `shell.uuidAutoGeneratedPlaceholder` (new) and the existing
> `shell.selectPlaceholder` (reused — values mirror content's own
> `uuidAutoGeneratedPlaceholder`/`selectPlaceholder` keys, kept separate per the
> shell/content namespace boundary). Wired `SubField` with `useTranslation()`.
