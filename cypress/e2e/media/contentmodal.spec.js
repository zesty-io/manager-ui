describe("Content media selection modal", () => {
  before(() => {
    ...
    cy.contains("Starting Content Editor").should("not.exist");
  ...
  it("limits selected media", () => {
    ...
    // Assert that we can check a media item
    cy.get("[data-testid='media-thumbnail-content']")
      .eq(0)
      //.first()
      .parent()
  ...
  it("locks nav to locked media group", () => {
    ...
    //cy.contains('Loading instance').should('not.exist');
    //cy.contains('Checking your account permissions').should('not.exist');
    //cy.contains('Starting Content Editor').should('exist');
    //cy.contains('Starting Content Editor').should('not.exist');
    cy.contains("Creating New Item").should("exist");
    cy.contains("Creating New Item").should("not.exist");
    //});
