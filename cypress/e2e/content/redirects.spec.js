...
describe("Content item redirects", () => {
  ...
  // retries:0 — a retry re-runs the create, but the attempt-1 redirect (<1s old)
  // isn't queryable yet so the pre-create cleanup misses it and the POST 400s on a
  // duplicate. No retry = no second create. (The cleanup below still handles any
  // cross-run leftover, which is old enough to be queryable.)
  it("Redirect Content Item", { retries: 0 }, () => {
  ...
  it("Stop Content Item Redirect", { retries: 0 }, () => {
