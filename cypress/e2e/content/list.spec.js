...
describe("Content List Actions", () => {
  ...
  it("Saves bulk edits", () => {
    ...
    // Settle on observable UI state instead of an exact count of background
    // refetches (the old `cy.wait(["@itemRefetch","@itemRefetch"])` failed
    // whenever the app fired a different number of GETs, and let in-flight GETs
    // bleed into the next test). The Save button only renders while changes are
    // staged, so its removal confirms staging cleared; the toggle then reflects
    // the refetched saved value, so aria-pressed on the "1" button confirms the
    // refetch landed before the next test runs.
