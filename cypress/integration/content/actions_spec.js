describe("Actions in content editor", () => {
  ...
  it("Must not save when missing required Field", () => {
    ...
    cy.get("input[name=text_field]", { timeout: 25000 }).clear();
  ...
  it("Save when missing required deactivated field", () => {
    ...
    // Provide large timeout for API calls
    cy.get("input[name=title]", { timeout: 25000 })
      .clear({ force: true }, { timeout: 25000 })
      .type(timestamp);
  ...
  it("Saves homepage item metadata", () => {
    ...
    // go to Meta Tab
    // Provide large timeout for API calls
    cy.get("[data-cy=meta]", { timeout: 25000 }).click();
    cy.get("textarea")
      .first()
  ...
  it("Creates a new item", () => {
    ...
    cy.get("input[name=title]", { timeout: 25000 }).click().type(timestamp);
  ...
  it("Displays a new item in the list", () => {
    ...
    cy.contains(timestamp, { timeout: 25000 }).should("exist");
  });

  it("Deletes an item", () => {
    cy.contains(timestamp).click();
    cy.get("article.Delete", { timeout: 25000 }).click();
